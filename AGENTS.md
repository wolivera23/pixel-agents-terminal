...
server/                       — Standalone server (Node.js, no VS Code deps except types)
  src/
    ...
    providers/file/
      claudeHookInstaller.ts  — Install/uninstall hooks in ~/.Codex/settings.json
      hooks/Codex-hook.ts    — Hook script: reads stdin, POSTs to server (bundled to CJS by esbuild)
  __tests__/                  — Vitest test suite
    ...
    Codex-hook.test.ts       — Integration: spawns real hook script process
...
**Dual-mode session detection**: Hooks mode (preferred) uses Codex Hooks API for instant, reliable detection. Heuristic mode (fallback) uses filesystem polling when hooks are unavailable. The `hookDelivered` flag per agent and `hooksEnabledRef` globally control the switch.

**Hooks mode** (11 events): `SessionStart` (session begin/resume/clear), `SessionEnd` (exit/clear), `Stop` (turn complete), `PermissionRequest`, `Notification` (idle/permission prompt), `UserPromptSubmit` (instant agent spawn confirmation), `PreToolUse` (instant active state), `PostToolUse`, `PostToolUseFailure`, `SubagentStart`, `SubagentStop`. HTTP server (`server/src/server.ts`) receives events via `~/.pixel-agents/hooks/Codex-hook.js`. Server discovery via `~/.pixel-agents/server.json` (port + PID + auth token). Multi-window safe. When hooks are active, heuristic scanners (main 1s, external 3s, stale 30s) are skipped entirely.
...
- Hook script (`Codex-hook.ts`) bundled to standalone CJS via esbuild (`buildHooks()` in esbuild.js), output: `dist/hooks/Codex-hook.js`
